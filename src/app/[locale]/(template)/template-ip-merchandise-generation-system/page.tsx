...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({
    locale,
    namespace: 'templates.ipMerchandiseGenerationSystem.seo',
  })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-ip-merchandise-generation-system`
          : `${getSiteUrl()}/template-ip-merchandise-generation-system`,
    },
  }
}
...
export default async function IPMerchandiseGenerationSystemLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.ipMerchandiseGenerationSystem' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-ip-merchandise-generation-system`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <LandingFooter />
      <JsonLd
        id="ip-merchandise-generation-system-schema"
