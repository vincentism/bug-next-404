...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({
    locale,
    namespace: 'templates.accessoriesModelReproduction.seo',
  })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-accessories-model-reproduction`
          : `${getSiteUrl()}/template-accessories-model-reproduction`,
    },
  }
}
...
export default async function AccessoriesModelReproductionLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.accessoriesModelReproduction' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-accessories-model-reproduction`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <LandingFooter />
      <JsonLd
        id="accessories-model-reproduction-schema"
