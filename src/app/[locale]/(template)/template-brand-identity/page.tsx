...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.brandIdentity.seo' })
  ...
  return {
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-brand-identity`
          : `${getSiteUrl()}/template-brand-identity`,
    },
  }
}

export default async function BrandIdentityLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.brandIdentity' })
  const faqItems: FAQItem[] = t.raw('faq.items')
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-brand-identity`

  const outputImages = [
    'https://ik.imagekit.io/opencreator/web/xm/1121-templates-covers/Brand Identity.png',
  ]
