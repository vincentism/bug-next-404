...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.christmasSalesPoster.seo' })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-christmas-sales-poster`
          : `${getSiteUrl()}/template-christmas-sales-poster`,
    },
  }
}
...
export default async function ChristmasSalesPosterLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.christmasSalesPoster' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-christmas-sales-poster`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <LandingFooter />
      <JsonLd
        id="christmas-sales-poster-schema"
