...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.adsRemake.seo' })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-ads-remake`
          : `${getSiteUrl()}/template-ads-remake`,
    },
  }
}

const TEMPLATE_ADS_REMAKE_OUTPUTVIDEOS_1 = [
...
export default async function AdsRemakeLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.adsRemake' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-ads-remake`
