...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.modelPosesIdeation.seo' })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-model-poses-ideation`
          : `${getSiteUrl()}/template-model-poses-ideation`,
    },
  }
}

export default async function ModelPosesIdeationLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.modelPosesIdeation' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-model-poses-ideation`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <WorkflowHero
        ...
        inputType="text"
        outputImages={TEMPLATE_MODEL_POSES_IDEATION_OUTPUTIMAGES_1}
        outputImageAlts={[
      ...
      <ModelShowcase />
      <WorkflowHowItWorksSection
        workflowKey="model-poses-ideation"
      ...
      <LandingFooter />
      <JsonLd
        id="model-poses-ideation-schema"
