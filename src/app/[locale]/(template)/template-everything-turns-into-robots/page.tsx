...
const OUTPUT_VIDEO =
  'https://ik.imagekit.io/opencreator/web/xm/0110/7b21dbe7880b7bc3409e2cb1e19c958f.mp4'

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.everythingTurnsIntoRobots.seo' })
  const canonical =
    locale === 'zh'
      ? `${getSiteUrl()}/zh/template-everything-turns-into-robots`
      : `${getSiteUrl()}/template-everything-turns-into-robots`
  return {
    title: t('title'),
...
export default async function EverythingTurnsIntoRobotsLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.everythingTurnsIntoRobots' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-everything-turns-into-robots`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <LandingFooter />
      <JsonLd
        id="everything-turns-into-robots-schema"
