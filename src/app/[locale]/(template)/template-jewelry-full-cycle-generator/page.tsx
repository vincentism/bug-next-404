...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.jewelryFullCycleGenerator.seo' })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-jewelry-full-cycle-generator`
          : `${getSiteUrl()}/template-jewelry-full-cycle-generator`,
    },
  }
}

const TEMPLATE_JEWELRY_FULL_CYCLE_GENERATOR_OUTPUTVIDEOS_1 = [OUTPUT_VIDEO]

const TEMPLATE_JEWELRY_FULL_CYCLE_GENERATOR_OUTPUTIMAGES_1 = [COVER_IMAGE]

type PageProps = {
  params: Promise<{ locale: string }>
}

export default async function JewelryFullCycleGeneratorLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.jewelryFullCycleGenerator' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-jewelry-full-cycle-generator`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <WorkflowHero
        ...
        inputType="image"
        outputVideos={TEMPLATE_JEWELRY_FULL_CYCLE_GENERATOR_OUTPUTVIDEOS_1}
        outputImages={TEMPLATE_JEWELRY_FULL_CYCLE_GENERATOR_OUTPUTIMAGES_1}
        outputImageAlts={[
      ...
      <LandingFooter />
      <JsonLd
        id="jewelry-full-cycle-generator-schema"
