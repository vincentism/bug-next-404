...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.instructionFlowchartMaker.seo' })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-instruction-flowchart-maker`
          : `${getSiteUrl()}/template-instruction-flowchart-maker`,
    },
  }
}
...
export default async function InstructionFlowchartMakerLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.instructionFlowchartMaker' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-instruction-flowchart-maker`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <LandingFooter />
      <JsonLd
        id="instruction-flowchart-maker-schema"
