...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.cosmeticsECommercePhotoSet.seo' })
  ...
  return {
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-cosmetics-e-commerce-photo-set`
          : `${getSiteUrl()}/template-cosmetics-e-commerce-photo-set`,
    },
  }
}

export default async function CosmeticsPhotoSetLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.cosmeticsECommercePhotoSet' })
  const faqItems: FAQItem[] = t.raw('faq.items')
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-cosmetics-e-commerce-photo-set`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <ModelShowcase />
      <WorkflowHowItWorksSection
        workflowKey="cosmetics-e-commerce-photo-set"
