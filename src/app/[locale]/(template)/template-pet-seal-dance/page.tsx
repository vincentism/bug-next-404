...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.petSealDance.seo' })
  const canonical =
    locale === 'zh'
      ? `${getSiteUrl()}/zh/template-pet-seal-dance`
      : `${getSiteUrl()}/template-pet-seal-dance`
  return {
    title: t('title'),
...
export default async function PetSealDanceLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.petSealDance' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-pet-seal-dance`
