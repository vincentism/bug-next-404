...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.hatModelAdvertisement.seo' })
  ...
  return {
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-hat-model-advertisement`
          : `${getSiteUrl()}/template-hat-model-advertisement`,
    },
  }
}
...
export default async function HatModelAdvertisementLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.hatModelAdvertisement' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-hat-model-advertisement`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <LandingFooter />
      <JsonLd
        id="hat-model-advertisement-schema"
