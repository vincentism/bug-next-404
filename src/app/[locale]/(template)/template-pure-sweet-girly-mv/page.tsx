...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.pureSweetGirlyMv.seo' })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-pure-sweet-girly-mv`
          : `${getSiteUrl()}/template-pure-sweet-girly-mv`,
    },
  }
}
...
export default async function PureSweetGirlyMvLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.pureSweetGirlyMv' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-pure-sweet-girly-mv`

  const outputImages = ['https://ik.imagekit.io/opencreator/web/xm/0201/2月1日 (2).png']
