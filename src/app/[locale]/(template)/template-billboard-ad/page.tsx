...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.billboardAd.seo' })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-billboard-ad`
          : `${getSiteUrl()}/template-billboard-ad`,
    },
  }
}

export default async function BillboardAdLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.billboardAd' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-billboard-ad`

  const outputVideos = ['https://ik.imagekit.io/opencreator/web/xm/1120-home-page-hovers/1 (11).mp4']
  const outputImages = [
    'https://ik.imagekit.io/opencreator/web/xm/1119-templates-refresh/Billboard Ad.png',
  ]
