...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.ninePanelStoryboard.seo' })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-nine-panel-storyboard`
          : `${getSiteUrl()}/template-nine-panel-storyboard`,
    },
  }
}
...
export default async function NinePanelStoryboardLandingPage({ params }: PageProps) {
  const { locale } = await params
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <LandingFooter />
      <JsonLd
        id="nine-panel-storyboard-schema"
