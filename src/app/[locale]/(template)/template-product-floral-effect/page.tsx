...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.productFloralEffect.seo' })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-product-floral-effect`
          : `${getSiteUrl()}/template-product-floral-effect`,
    },
  }
}

export default async function ProductFloralEffectLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.productFloralEffect' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-product-floral-effect`

  const outputVideos = ['https://ik.imagekit.io/opencreator/web/xm/1120-home-page-hovers/1 (14).mp4']
  const outputImages = [
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <LandingFooter />
      <JsonLd
        id="product-floral-effect-schema"
