...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.studioLevelPhotography.seo' })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-studio-level-photography`
          : `${getSiteUrl()}/template-studio-level-photography`,
    },
  }
}

export default async function StudioLevelPhotographyLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.studioLevelPhotography' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-studio-level-photography`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <LandingFooter />
      <JsonLd
        id="studio-level-photography-schema"
