...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const tSeo = await getTranslations({ locale, namespace: 'templates.polaroidArtCollage.seo' })
  ...
  return {
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-polaroid-art-collage`
          : `${getSiteUrl()}/template-polaroid-art-collage`,
    },
  }
}

export default async function PolaroidArtCollageLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.polaroidArtCollage' })
  const faqItems = t.raw('faq.items') as FAQItem[]
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-polaroid-art-collage`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <LandingFooter />
      <JsonLd
        id="polaroid-art-collage-schema"
