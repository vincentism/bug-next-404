...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.multiverseProblemSolver.seo' })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-multiverse-problem-solver`
          : `${getSiteUrl()}/template-multiverse-problem-solver`,
    },
  }
}

export default async function MultiverseProblemSolverLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.multiverseProblemSolver' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-multiverse-problem-solver`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <WorkflowHero
        ...
        inputType="text"
        outputImages={TEMPLATE_MULTIVERSE_PROBLEM_SOLVER_OUTPUTIMAGES_1}
        outputImageAlts={[
      ...
      <LandingFooter />
      <JsonLd
        id="multiverse-problem-solver-schema"
