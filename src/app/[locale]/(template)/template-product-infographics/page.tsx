...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.productInfographics.seo' })
  ...
  return {
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-product-infographics`
          : `${getSiteUrl()}/template-product-infographics`,
    },
  }
}

export default async function ProductInfographicsLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.productInfographics' })
  const faqItems: FAQItem[] = t.raw('faq.items')
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-product-infographics`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <ModelShowcase />
      <WorkflowHowItWorksSection
        workflowKey="product-infographics"
      ...
      <LandingFooter />
      <JsonLd
        id="product-infographics-schema"
