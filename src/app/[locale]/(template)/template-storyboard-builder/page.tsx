...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.storyboardBuilder.seo' })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-storyboard-builder`
          : `${getSiteUrl()}/template-storyboard-builder`,
    },
  }
}

export default async function StoryboardBuilderLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.storyboardBuilder' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-storyboard-builder`
