...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.brandLogoXMaterial.seo' })
  ...
  return {
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-brand-logo-x-material`
          : `${getSiteUrl()}/template-brand-logo-x-material`,
    },
  }
}

export default async function BrandLogoMaterialLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.brandLogoXMaterial' })
  const faqItems: FAQItem[] = t.raw('faq.items')
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-brand-logo-x-material`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <ModelShowcase />
      <WorkflowHowItWorksSection
        workflowKey="brand-logo-x-material"
