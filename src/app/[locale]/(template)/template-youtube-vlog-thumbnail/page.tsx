...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'templates.youtubeVlogThumbnail.seo' })
  return {
    title: t('title'),
    ...
    alternates: {
      canonical:
        locale === 'zh'
          ? `${getSiteUrl()}/zh/template-youtube-vlog-thumbnail`
          : `${getSiteUrl()}/template-youtube-vlog-thumbnail`,
    },
  }
}

export default async function YouTubeVlogThumbnailLandingPage({ params }: PageProps) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'templates.youtubeVlogThumbnail' })
  const siteUrl = getSiteUrl()
  const pageUrl = `${siteUrl}/${locale}/template-youtube-vlog-thumbnail`
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden bg-[#F7F7F7]">
      ...
      <LandingFooter />
      <JsonLd
        id="youtube-vlog-thumbnail-schema"
